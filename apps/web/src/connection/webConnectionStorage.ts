...
function makeCatalogBackend(database: IDBDatabase): WebCatalogBackend {
...
const makeCatalogStore = Effect.fn("web.connectionStorage.makeCatalogStore")(function* (
  ...
  const update: WebCatalogStore["update"] = Effect.fn("web.connectionStorage.updateCatalog")(
    function* (transform) {
  ...
  return { read, update } satisfies WebCatalogStore;
});

export const webConnectionStorageLayer = Layer.effectContext(
  Effect.gen(function* () {
